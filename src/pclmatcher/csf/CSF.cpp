...
void CSF::setLivoxPointcloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    point_cloud.clear();
    point_cloud.resize(cloud->points.size());

    for (size_t i = 0; i < cloud->points.size(); ++i) {
        csf::Point las;
        las.x = cloud->points[i].x;
        las.y = -cloud->points[i].z;
        las.z = cloud->points[i].y;
        point_cloud[i] = las;
    }
}
